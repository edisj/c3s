...
class TestBinarySystem:
    ...
    def binary_many_body(self):
        system = c3s.simulators.MasterEquation(cfg='config_files/binary.yml', A=3)
        start, stop, step = 0, 1, 0.01
        system.run(start, stop, step)
        yield system
...
class Test2IsolatedSwitch:
    ...
    def isolated_switches(self):
        system = c3s.simulators.MasterEquation(cfg='config_files/2_isolated_switches.yml', A=1, B=1)
        start, stop, step = 0, 1, 0.01
        system.run(start, stop, step)
        yield system
